...
public class MemberController {
	...
	public String join(
		...
		if(memberSVC.isExistEmail(joinForm.getEmail())) {
			bindingResult.reject("error.member.join","이미 사용중인 이메일입니다.");
			return "member/joinForm";
		}
		
		//비밀번호 확인 체크
		if(!joinForm.getPw().equals(joinForm.getPwchk())) {
			bindingResult.reject("err.member.join","비밀번호가 일치하지 않습니다.");
		}
	...
	public String editForm(HttpServletRequest request,Model model) {
		...
		if(memberDTO.getLetter()=="1") {editForm.setLetter(true);}else{editForm.setLetter(false);}
//		memberDTO.getLetter().compareTo(String "1") ? editForm.setLetter(true) : editForm.setLetter(false);
